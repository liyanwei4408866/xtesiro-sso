define(
		[ 'jquery', 'views/communication-base-view','text!../../templates/usergroup/usergroupAddTemplate.html','views/room/upload-snippet-view'],
		function($, CommunicationBaseView,Template,UploadSnippetView) {
			var treeObj;
			var datatable;
			var UsergroupAddSnippetView = CommunicationBaseView
					.extend({
						events : {
						...
						_setContentHTML : function (room){
							...
							this.$el.append(html);
							var commonDialog;
							var height = document.documentElement.clientHeight/1.2>150?150:document.documentElement.clientHeight/1.2;
							if(room)
								commonDialog=fh.commonOpenDialog('roomAddDialog', "编辑用户组", 750, height, this.el);
							else
								commonDialog=fh.commonOpenDialog('roomAddDialog', "新增用户组", 750, height, this.el)
			...
			return UsergroupAddSnippetView;
		});