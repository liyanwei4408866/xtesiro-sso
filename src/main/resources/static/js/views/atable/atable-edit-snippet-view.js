...
(
  ...
  function (
	...
	Template) {
	  var ATableEditSnippetView = CommunicationBaseView.extend({
		events : {
		},
		initialize : function() {
			this.childView = {};
		...
		render : function(parentView,atableObj) {
			...
			this.viewConstant = this.parentView.viewConstant.dialog.edit;
		...
		_initData:function(atableObj){
			var self = this;
			if(atableObj){// 修改
				self.$el.find("#usergroupId").val(atableObj.usergroupId);
				self.$el.find("#usergroupName").val(atableObj.usergroupName);
			}
		},
