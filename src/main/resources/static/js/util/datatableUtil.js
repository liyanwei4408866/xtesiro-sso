...
//			            aoData=togetherParams(aoData, tableObj, parmObj,settings);
			            aoData=togetherParams(aoData, tableObj, "",settings);
...
//			                    + "&sort=" + sortName + " " + sort;
			            var paramData = {
			            	current : Math.ceil(Math.ceil(parseInt(settings._iDisplayStart) / parseInt(settings._iDisplayLength)) +1),
			            	display : settings._iDisplayLength,
			            	sort : sortName + " " + sort
			            }
			            var requestData = $.extend( true, parmObj, paramData );
			            //params
			            settings.jqXHR = $.ajax({
			            	"type":"GET",
			                "url": tableObj.url,
